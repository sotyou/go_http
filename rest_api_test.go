    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            if len(tt.args.data) > 0 {
                req = _newRequest("POST", "", bytes.NewReader([]byte(tt.args.data)))
            } else {
                req = _newRequest("POST", "", nil)
            }
            got, err := RequestBody(req)
            // 是否报错
            if (err != nil) != tt.wantErr {
                t.Errorf("RequestBody() error = %v, wantErr %v", err, tt.wantErr)
                return
            }
            // 是否取到body
            if !reflect.DeepEqual(got, []byte(tt.args.data)) {
                t.Errorf("RequestBody() got = %v, want %v", string(got), string(tt.args.data))
            }
            // request.Body 是否能读取
            got2, err := RequestBody(req)
            if (err != nil) != tt.wantErr {
                t.Errorf("Second RequestBody() error = %v, wantErr %v", err, tt.wantErr)
                return
            }
            if !reflect.DeepEqual(got2, []byte(tt.args.data)) {
                t.Errorf("Second RequestBody() got = %v, want %v", string(got), string(tt.args.data))
            }
            //pp.Println(string(got), string(got2), tt.args.data)
        })
    }
}
